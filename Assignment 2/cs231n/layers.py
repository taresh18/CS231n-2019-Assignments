...
def batchnorm_backward(dout, cache):
    ...
    N = dout.shape[0]
    [x_norm, num, denom, gamma] = cache
    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout*x_norm, axis=0)
    x_bar = np.ones(dout.shape)/N
    dx = dout * gamma
    dx2 = dx / denom
    dx1 = np.sum(dx * num, axis=0)
    dx1 = -1 * dx1/(denom**2)
    dx1 = (dx1 * 0.5) / denom
    dx1 = dx1 * x_bar
    dx1 = 2*dx1*num
    dx = dx1 + dx2
    dx1 = np.sum(-1 * dx, axis=0)
    dx1 = dx1 * x_bar
    dx = dx + dx1
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = dx/denom - gamma*np.sum(dout,axis=0)/(N*denom) - (gamma*num)/(denom**3)*np.sum(dout*num,axis=0)/N
    # *****END OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_backward(dout, cache):
    ...
    dgamma = np.sum(dout * x_norm, axis=0)
    dx = dout * gamma
    dx = dx/denom - (np.sum(dx,axis=1)[:,np.newaxis]/(D*denom)) - np.sum(dx*x_norm/denom,axis=1)[:,np.newaxis]*x_norm/D
        ...
        for i in range(0,H+2*pad,stride):
            if i+HH<=(H+2*pad):
                ...
                for j in range(0,W + 2*pad,stride):
                    if j+WW<=(W+2*pad):
                        k = x_pad[:,:,i:i+HH,j:j+WW]
                        fil = w[f:f+1,:,:,:]
                        k = k*fil
                        out[:,f:f+1,t:t+1,y:y+1] = np.sum(k,axis=(1,2,3),keepdims=True) + b[f]
                        y+=1
                t+=1
...
def max_pool_forward_naive(x, pool_param):
    ...
    for i in range(p_H):
        for j in range(p_W):
            out[:,:,i:i+1,j:j+1] = np.max(x[:,:,i*stride:i*stride + p_H,j*stride:j*stride + p_W],axis=(2,3),keepdims=True) 
...
def max_pool_backward_naive(dout, cache):
    ...
    for n in range(N):
        for c in range(C):
            for i in range(p_H):
                for j in range(p_W):
                    k = x[n,c,i*stride:i*stride + p_H,j*stride:j*stride + p_W]
                    ind = np.argmax(k)
                    ind = np.unravel_index(ind,(p_H,p_W))
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    N,C,H,W = x.shape
    xbar = x.reshape((N*G,(C//G)*H*W)).T
    mean = np.mean(xbar,axis=0)
    var = np.var(xbar,axis=0)
    num = xbar - mean
    denom = np.sqrt(var + eps)
    x_norm = num / denom
    x_norm = np.reshape(x_norm.T, x.shape)
    out = gamma * x_norm + beta
    cache = (gamma, denom, x_norm, G) 
...
def spatial_groupnorm_backward(dout, cache):
    ...
    gamma, denom, x_norm, G = cache
    N,C,H,W  = dout.shape
    dbeta = np.sum(dout,axis = (0,2,3), keepdims=True)
    dgamma = np.sum(x_norm * dout, axis = (0,2,3), keepdims=True)
    dx = dout*gamma
    shape = (N*G,(C//G)*H*W)
    dx = dx.reshape(shape).T
    x_norm = x_norm.reshape(shape).T
    K = shape[1]
    dx = dx/denom - (np.sum(dx,axis=0)/(K*denom)) - (np.sum((dx*x_norm)/denom,axis=0)*x_norm/K)
